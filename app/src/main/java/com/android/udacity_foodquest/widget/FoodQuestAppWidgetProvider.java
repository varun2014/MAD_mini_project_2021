...
public class FoodQuestAppWidgetProvider extends AppWidgetProvider {
    ...
    private static RemoteViews getRestaurantsFromListView(Context context) {
        RemoteViews views = new RemoteViews(context.getPackageName(), R.layout.foodquest_app_widget);

        Intent listViewIntent = new Intent(context, FoodQuestWidgetService.class);
    ...
    public void onReceive(Context context, Intent intent) {
        if (intent != null) {
            if (Objects.requireNonNull(intent.getAction()).equals(AppWidgetManager.ACTION_APPWIDGET_UPDATE)) {
                ...
                ComponentName thisWidget = new ComponentName(context.getApplicationContext(), FoodQuestAppWidgetProvider.class);
